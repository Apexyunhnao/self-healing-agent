...
class TestServices(unittest.TestCase):
    def test_services_lists_mock_and_local(self):
        cp = run_cli(["services", "--json"])
        self.assertEqual(cp.returncode, 0, cp.stderr)
        data = json.loads(cp.stdout)
        names = [s["name"] for s in data]
        self.assertIn("mock_svc", names)
        self.assertIn("local_svc", names)
    ...
    def test_services_text_output(self):
        ...
        self.assertIn("local_svc", cp.stdout)
...
class TestStatus(unittest.TestCase):
    ...
    def test_status_all_services_parseable(self):
        ...
        self.assertEqual(cp.returncode, 0, cp.stderr)
        data = json.loads(cp.stdout)
        self.assertGreaterEqual(len(data), 2)  # mock_svc + local_svc
