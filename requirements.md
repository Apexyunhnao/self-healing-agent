...
> 本文档是需求基线，成功指标必须数学自洽，写进 README 前先核对。

## 1. 项目概述

**Local Service Self-Healing Agent（本地服务自愈智能体）**：运行在本机的自愈编排器，持续监控关键服务，故障时由 LLM/规则诊断根因，经策略门校验后执行白名单修复动作，由确定性验证器证明修复成功；修复失败自动重试，预算耗尽升级人工并隔离。全程状态机驱动，故障生命周期可审计、可回放。
...
作者本机常年运行多个必须保持健康的服务（本地 HTTP 服务 / 消息网关 / 调试浏览器），现状：
- 服务挂了靠人发现、人手动重启（曾有多次崩溃经历）
- 已有 watchdog 脚本只告警不自动修复
- 企业视角：生产服务不可用 = 直接损失；SRE 核心诉求 = 自动发现→自动修复→证明修好→修不好升级人工
...
4. **执行**：白名单动作幂等执行（restart / kill_stale / cleanup_logs / git_revert_config / escalate）
5. **验证**：确定性验证器证明修复真实生效（重试+指数退避，防启动未就绪误判）
6. **兜底**：重试预算耗尽 → 升级人工 + quarantine，绝不无限重试
...
**自洽检查（实测口径，唯一来源 eval/eval_report.json）**：全量 35 场景（含盲评 6 个）检测率 28/28=100%、正确处理率 28/28=100%、修复成功率 17/28=60.7%（Rule-only；策略门主动升级 11 个，非修不好）、MTTR 均值 5.1s、误报 0/6、Unsafe 0/34、Escape 0/17。**LLM Incremental Value 实测 +0pp**（Mixed 5 个场景中 Rule 3/5 vs LLM 3/5，整体修复成功率 +4pp 但 MTTR +1.2s）——需求阶段"LLM 应比规则强 +40pp"的假设未成立，该结论由 A/B 对照实验得出。以上数字写文档前必须从 run_eval.py 报表变量读取，禁止模板写死。

## 5. 范围（v1 做）

- 监控对象（service.yaml 声明）：
  1. 本地 HTTP 服务（HTTP，占位端口 8080）
  2. 消息网关（进程 + DB 完整性）
  3. 调试浏览器（CDP）
...
- Python 3.11+，标准库 + httpx + pyyaml + psutil
- 手写状态机（13 状态：healthy/degraded/diagnosing/policy_check/repairing/starting/verifying/recovered/rolling_back/retry/escalated/awaiting_human/quarantined）
- service.yaml 声明式配置 + @probe / @action 注册机制（可扩展性证明）
- LLM 诊断：DeepSeek API；输出经 JSON 解析、字段完整性、置信度范围与动作白名单校验（DiagnosisResult 为 dataclass），解析或校验失败走 Rule Diagnoser，置信度 < 0.7 走保守动作
- 配置回退（可选动作）：白名单含 git_revert_config，对 git 管理的 config_dir 执行 git reset --hard HEAD~1；当前不在状态机自动转移路径内，作为可选能力保留
...
6. 复盘：README + tech_selection.md + dev-log
