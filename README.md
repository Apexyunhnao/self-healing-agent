...
诊断器：Rule-only 与 LLM+fallback 在**同一测试集、同一环境**下对照运行。**数字唯一来源**：`eval/eval_report.json`（由 `eval/run_eval.py --final --diagnoser llm` 自动生成，README 与简历只引用它，不手写数字）。**分母说明**（防"文字游戏"误读）：
