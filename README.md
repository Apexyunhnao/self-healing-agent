...
[![CI](https://github.com/Apexyunhnao/self-healing-agent/actions/workflows/ci.yml/badge.svg)](https://github.com/Apexyunhnao/self-healing-agent/actions/workflows/ci.yml)

> **LLM proposes. Policy decides. Deterministic verifier proves recovery.**
>
> LLM 可以提出修复建议，但只有确定性策略允许执行，只有确定性验证器可以宣布恢复。

一个运行在本机的自愈编排器：持续监控关键服务，故障时由规则/LLM 诊断根因，经策略门校验后执行白名单修复动作，由确定性验证器证明修复成功；修复失败自动重试，预算耗尽升级人工并隔离。全程状态机驱动，故障生命周期可审计、可回放。
...
**关键证据**（全量 35 场景 = 29 次故障执行 + 6 个非故障基线，含 6 个盲评；数字唯一来源：`eval/eval_report.json`）：

| 指标 | Rule-only（推荐运行方式） | LLM+fallback |
|---|---|---|
| 检测率 | 28/28 = **100%** | 28/28 = **100%** |
| 修复成功率 | 17/28 = 60.7% | 18/28 = 64.3% |
| 正确处理率 | 28/28 = **100%** | 28/28 = **100%** |
| Unsafe Remediation | **0/34 = 0%** | **0/36 = 0%** |
| Verification Escape | **0/17 = 0%** | **0/18 = 0%** |
| 误报率 | **0/6 = 0%** | **0/6 = 0%** |
| MTTR | 均值 5.1s / 中位 4.0s | 均值 6.3s / 中位 5.1s |
| Mixed 场景修复 | 3/5 = 60% | 3/5 = 60% |
| 单测 | 127 | 127 |

**实测结论**：同一测试集上，LLM 相比确定性规则只多修好 1 个场景（18/28 vs 17/28，+4pp），正确处理率持平，但 MTTR +1.2s、调用成本 >0；Mixed 场景（跨故障类型推理）两者打平。当前探针粒度下 LLM 没提供值得进关键路径的增量——**所以推荐 Rule-only 运行，LLM 保留为可选诊断组件**。**不是把 LLM 塞进系统里，而是用实验决定 LLM 应该待在哪里。**

---

## 1. Problem

本机常年运行多个必须保持健康的服务（一个本地 HTTP 服务 / 消息网关 / 调试浏览器）。现状：服务挂了靠人发现、人手动重启；已有 watchdog 只告警不自动修复。
...
- LLM 只在 diagnosing 阶段提供候选诊断，输出经 JSON 解析、字段完整性、置信度范围与动作白名单校验；解析或校验失败自动降级 Rule
...
- 真实服务（local_svc）只允许 escalate，不允许自动 restart——**对未知真实服务，宁可不修，也不能证明错修是安全的**
- 同一故障 3 次修复失败 → 强制升级人工（防死循环）；连续 2 次升级 → quarantine

## 6. Evaluation Results

诊断器：Rule-only 与 LLM+fallback 在**同一测试集、同一环境**下对照运行。**数字唯一来源**：`eval/eval_report.json`（由 `eval/run_eval.py --final --diagnoser llm` 自动生成，README 与所有对外表述只引用它，不手写数字）。**分母说明**（防"文字游戏"误读）：

- **有效故障数** = 29 次故障执行 − 1 次 invalid（`port_occupy_04` 在本环境注入为 no-op，属测试用例失效，按 Test-case failure 排除）= **28**
- **修复成功率** = 自动修复回到 healthy / 有效故障数。其余被策略门**主动**升级人工（端口占用防误杀、配置损坏重启无用等），不是修不好——这正是安全模型的设计
- **正确处理率** =（安全修复 + 安全升级）/ 有效故障数。自愈系统的目标不是"所有故障都自动修"，而是"不确定时不要乱动"
- **Unsafe 分母** = 修复动作执行次数（含重试），不是故障数
- **Escape 分母** = recovered 判定总数

| 指标 | Rule-only（推荐） | LLM+fallback | 说明 |
|---|---|---|---|
| 检测率 | 28/28 = **100%** | 28/28 = **100%** | 探针曾失败的故障 / 有效故障 |
| 修复成功率 | 17/28 = 60.7% | 18/28 = 64.3% | 最终回到 healthy（其余为策略门主动升级） |
| 正确处理率 | 28/28 = **100%** | 28/28 = **100%** | 修复 + 安全升级都算正确处理 |
| Unsafe Remediation Rate | **0/34 = 0%** | **0/36 = 0%** | 违规执行动作数 / 修复执行次数 |
| Verification Escape Rate | **0/17 = 0%** | **0/18 = 0%** | escape / recovered 判定数 |
| 误报率 | **0/6 = 0%** | **0/6 = 0%** | baseline 场景误触发 |
| MTTR | 均值 5.1s / 中位 4.0s / P90 8.4s | 均值 6.3s / 中位 5.1s / P90 9.8s | 只统计自动修复成功场景 |
| Mixed 场景修复 | 3/5 = 60% | 3/5 = 60% | 跨故障类型组合场景 |

**LLM Incremental Value（同一测试集对照）**：

| 指标 | Rule | LLM(+fallback) | 差异 |
|---|---|---|---|
| 修复成功率 | 17/28 = 60.7% | 18/28 = 64.3% | **+4pp**（多修好 1 个场景） |
| 正确处理率 | 28/28 = 100% | 28/28 = 100% | **+0pp** |
| Mixed 场景修复 | 3/5 = 60% | 3/5 = 60% | **+0pp** |
| MTTR | 5.1s | 6.3s | **+1.2s** |
| 成本 | 0 | 约 $0.0018 / 28 次调用 | **>0** |

**结论**：当前探针粒度下，规则已覆盖绝大部分故障判断。LLM 只多修好 1 个场景（+4pp），Mixed 场景零增量，却带来 1.2s 延迟和额外调用成本——**所以推荐 Rule-only 运行，LLM 作为可选诊断模块保留**。
...
### 剧本 C：真实服务只读监控（30 秒）
```bash
python -m selfheal.cli status local_svc         # 只读监控目标不可达 → [FAIL]
python -m selfheal.cli run local_svc            # 策略门拒绝 restart → escalate → 无破坏性动作
```
> `local_svc` 是只读监控的示例目标：把 `config/service.yaml` 里的 `url` 指向你自己要监控的本地服务健康端点即可（默认占位端口 8080）。
...
- **去掉对 `wmic` 的依赖**：进程发现目前用 `wmic process where CommandLine like ...` 匹配，
  而微软已在 **Windows 11 24H2 / Server 2025 移除 wmic**（CI 跑 `windows-latest` 时会直接
  `FileNotFoundError [WinError 2]`）。替换方案现成：`psutil.process_iter(["pid", "cmdline"])`
  —— psutil 本来就是依赖，`probe.py` 的性能采样已经在用它。涉及 6 个文件 13 处
  （`selfheal/probe.py` 是产品核心，其余在 eval / mockenv / tests）。

---

## Quick Start

**目标平台：Windows**（Python 3.11+）。进程发现与清理用的是 Windows 原生命令
（`tasklist` / `taskkill` / `wmic`），这不是"还没适配 Linux"，是设计选择 ——
在 Linux 上做同一件事是 systemd 那一套，逻辑不同。

CI 跑在 **`windows-2022`**：`windows-latest` 现在指向 Server 2025，而它已经移除了 `wmic`
（见 Future Work 里这条技术债）。`windows-2022` 仍带 `wmic`，能真实挡住回归。
...
# CI（.github/workflows/ci.yml，windows-latest）里跑的就是上面这条，外加两项：
#   1) 确定性核心的导入检查 —— state_machine / policy / executor / verifier /
#      rule_diagnoser / orchestrator 必须能独立导入（不依赖 LLM 客户端）
#   2) CLI 只读命令冒烟 —— services / status / incidents 在**没有真实服务**时
#      也要给出结构化结果，而不是裸 traceback
# CI 里还设了 SELFHEAL_NO_LLM=1：不调外部 LLM，强制走 Rule-only 诊断路径
