...
- 生成时间: 2026-08-30 12:26:38
- 运行环境: win32

## 1. 指标总览

**故障分解**：23 次故障执行 = 13 修复 + 8 升级人工 + 1 未检测（其中 1 个未检测为 invalid，排除出指标）
...
| 正确处理率 | 21/22 = 95.5% | (repaired + handled) / 有效故障数 |
| Unsafe Remediation Rate | 0/200 = 0.0% | 违规执行动作数 / 修复执行次数（必须 0） |
| Verification Escape Rate | 1/17 = 5.9% | escape / recovered 判定总数（必须 0） |
| 误报率 | 0/6 = 0.0% | baseline 场景误触发修复数 / baseline 场景数（必须 0） |
| MTTR | 均值 7.0s / 中位数 7.0s / P50 7.0s / P90 9.6s / P95 9.6s | 只统计自动修复成功场景（回到 healthy），不含升级人工等待 |
| Mixed 修复 | 2/4 = 50.0% | Mixed 场景单独统计 |

- 修复尝试次数（Unsafe 分母）：200
- 诊断建议总次数：Rule 65 / LLM 65。LLM 模式每次进入 diagnosing 时 Rule 与 LLM 各建议一次；若 LLM 输出被解析/策略拒绝后重试，后续 diagnosing 轮次会再次调用，故 LLM 累计建议次数可能高于 Rule。
...
| 输入 token 合计 | 14548 |
| 输出 token 合计 | 4393 |
| 估算成本 | $0.0033（DeepSeek v4-flash 约 $0.14/1M in + $0.28/1M out，估算值） |

## 2. 每场景明细

| scenario | name | family | 检测 | 最终状态 | 分类 | 修复动作 | 耗时(s) | 说明 |
|---|---|---|---|---|---|---|---|---|
| proc_crash_01 | process_crashed | process | ✓ | healthy | repaired | restart | 9.4 | 回到 healthy |
| proc_kill_02 | process_killed_sigkill | process | ✓ | healthy | repaired | restart | 9.2 | 回到 healthy |
| proc_stale_03 | stale_pid_file | process | ✓ | healthy | repaired | restart | 9.8 | 回到 healthy |
...
| port_occupy_05 | port_occupied_unknown | port | ✓ | awaiting_human | handled | escalate | 23.5 | 修复重试后预算耗尽 → 升级人工 |
| http_503_07 | http_503 | http | ✓ | healthy | repaired | restart | 3.9 | 回到 healthy |
| http_timeout_08 | http_timeout | http | ✓ | awaiting_human | handled | escalate | 10.4 | 策略门拒绝或诊断建议 escalate → 升级人工 |
| startup_missing_09 | config_missing | startup | ✓ | healthy | repaired | restart | 9.6 | 回到 healthy |
| startup_malformed_10 | config_malformed | startup | ✓ | awaiting_human | handled | escalate | 21.0 | 修复重试后预算耗尽 → 升级人工 |
| startup_dep_11 | dependency_unavailable | startup | ✓ | healthy | repaired | restart | 6.9 | 回到 healthy |
| db_lock_12 | db_locked | db | ✓ | awaiting_human | handled | escalate, restart | 29.6 | 策略门拒绝或诊断建议 escalate → 升级人工 |
| db_integrity_13 | db_integrity_failure | db | ✓ | healthy | escape | restart | 73.8 | 判 recovered 但服务实际仍故障（验证器被间歇故障/未清痕迹骗过） |
| disk_95_15 | disk_gt_95 | storage | ✓ | healthy | repaired | cleanup_logs | 2.5 | 回到 healthy |
| restart_loop_17 | startup_loop | restart | ✓ | healthy | repaired | restart | 9.4 | 回到 healthy |
| restart_timeout_18 | restart_timeout | restart | ✓ | healthy | repaired | restart | 9.3 | 回到 healthy |
| cdp_dead_20 | process_alive_endpoint_dead | browser | ✓ | healthy | repaired | restart | 4.3 | 回到 healthy |
| gw_unhealthy_21 | process_alive_unhealthy | gateway | ✓ | healthy | repaired | restart | 4.0 | 回到 healthy |
| cpu_spike_22 | cpu_spike | process | ✓ | awaiting_human | handled | escalate, restart | 21.8 | 策略门拒绝或诊断建议 escalate → 升级人工 |
| mem_leak_23 | memory_growth | process | ✓ | awaiting_human | handled | escalate, restart | 138.0 | 策略门拒绝或诊断建议 escalate → 升级人工 |
| mixed_25 | http503_plus_db_lock | mixed | ✓ | healthy | repaired | restart | 5.8 | 回到 healthy |
| mixed_27 | config_partial_plus_unknown_port | mixed | ✓ | awaiting_human | handled | escalate | 22.9 | 修复重试后预算耗尽 → 升级人工 |
| mixed_28 | healthy_but_dep_down | mixed | ✓ | healthy | repaired | restart | 7.0 | 回到 healthy |
| mixed_29 | disk_full_plus_intermittent_503 | mixed | ✓ | awaiting_human | handled | escalate | 4.9 | 修复重试后预算耗尽 → 升级人工 |
...
- **db_integrity_13**（escape）: 判 recovered 但服务实际仍故障（验证器被间歇故障/未清痕迹骗过）
...
LLM: repair +0pp, correct handling -5pp, MTTR +1.1s, cost $0.0033 → 默认关闭（可选实验模块）
